...
const float zeroWindAdjustment = 0.334; // negative numbers yield smaller wind speeds and vice versa.
...
void setup() {
  ...
  Serial.begin(38400);   // faster printing to get a bit better throughput on extended info
...
void loop() {
  ...
  if (millis() - lastMillis > 500){      // read every 200 ms - printing slows this down further
   ...
   WindSpeed_cmps = WindSpeed_MPH*0.44704*100 ;  //Prints windspeed in mm/s   
    ...
    Serial.print("   WindSpeed cm/s ");
    Serial.println((float)WindSpeed_cmps);
